...
This assignment contains two parts: the first part is a set of Airflow DAGs to scrape metadata from GOES-18 and NEXRAD S3 buckets and store the data in a Google Cloud Storage bucket, and the second part is a web application that allows users to subscribe to a plan, which will limit their API calls. The calls will aid search and download the required files using the scraped data.
